...
public class ConversationController {
    ...
    public ResponseEntity<ConversationDTO> getById(@PathVariable UUID uuid){
    ...
    public ResponseEntity<ConversationDTO> updateConversation(@PathVariable UUID uuid, @RequestBody UpdateConversationDTO dto){
        ConversationDTO updated = _conversationService.update(uuid, dto);
        return ResponseEntity.ok(updated);
    }

    @DeleteMapping("/delete/{uuid}")
    public ResponseEntity<Void> deleteConversation(@PathVariable UUID uuid){
        _conversationService.deleteById(uuid);
        return ResponseEntity.noContent().build();
    }

    @PatchMapping("/archived/{uuid}")
    public Boolean isArchived(@PathVariable UUID uuid, @RequestBody ArchivedConversationDTO dto){
